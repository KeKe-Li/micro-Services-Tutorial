...
在开发的过程中相信  用的都是官方提供的公共的镜像仓库，但是当企业应用的时候为了安全和效率等方面考虑，我们会部署自己的私有仓库Registry。Harbor就是一个这样的开源的企业级的Docker Registry管理项目,它包括权限管理(RBAC)、LDAP、日志审核、管理界面、自我注册、镜像复制和中文支持等功能。
